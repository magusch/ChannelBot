...
def vk_post(msg, attachments):
    site = "https://api.vk.com/method/wall.post"
    data = {
...
def create_vk_post(post):
    ...
    url_list = re.findall(
        r"(\((?:\[??[^\[]*?\)))", post[post.rfind("(http") :]
    )  # TODO: rebuild this stupid
    if len(url_list) > 0:
        url = url_list[0][1:-1]
    else:
        url = ""
...
class VKUpload:
    def __init__(
        ...
        self.get_end_str = (
            f"&access_token={token}&expires_in=86400&user_id={user_id}&v=5.103"
        )
