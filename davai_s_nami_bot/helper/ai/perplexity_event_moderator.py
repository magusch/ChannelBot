...
class PerplexityEventModerator:
    ...
    def moderate_events(self, events_list=[], example_events=[]):
        ...
        if self.system_message:
            ...
            ai_messages.append({
                "role": "system", "content": f"Вы модератор мероприятий для платформы по интересным, движовым и молодёжным мероприям {settings.city_name_gen}. Средний возраст читателей: 17-29 лет."
            })
    ...
    def _generate_prompt_with_events(self, events_list, max_events):
        ...
        """
        instructions = (
            f"""Твоя задача — оценить каждое мероприятие по нескольким критериям на 10-балльной шкале. Вот критерии оценки:
                        ...
                        5. Соответствие аудитории (1-10): Насколько мероприятие интересно аудитории канала? Аудитория это молодёжь и студенты {settings.city_name_gen}, возраст 17-29 лет. Мероприятия для детей, пенсионеров, людей с ограниченными возможностями здоровья получают низкие баллы.
                        6. Актуальность (1-10): Связь с трендами, сезонностью, текущими событиями. Насколько мероприятие актуально в текущий момент? Сезонные и модные темы получают больше баллов. Допустим мероприятие о фильме который сейчас в новостях получает больше баллов. 
    ...
    def _generate_prompt_with_examples(self, examples):
        ...
        """
        instructions = (
            """Обрати также внимания на примеры удачных мероприятий, которые уже были опубликованы в ближайшее время и учти их в своём критерии (можно добавить балы в некоторых пунтках):"""
        )
