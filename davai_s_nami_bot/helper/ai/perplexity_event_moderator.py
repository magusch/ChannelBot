...
class PerplexityEventModerator:
    def __init__(self, max_events_percent=30, max_moderate_percent=50):
        ...
        self.ai_model = param.site_parameters('perplexity_model', last=1) or "sonar-pro"
        self.answer = None
    ...
    def moderate_events(self, events_list=[], example_events=[]):
        ...
        if self.system_message:
            ...
            ai_messages.append({
                "role": "system", "content": "Вы модератор мероприятий для платформы по интересным, движовым и молодёжным мероприям Санкт-Петербурга. Средний возраст читателей: 17-29 лет."
            })
        
        if self.user_message:
            ai_message += self.user_message + "\n"
        max_events = self.max_events_percent * 0.01 * len(filetered_event_list)

        ai_message += self._generate_prompt_with_events(filetered_event_list, max_events) + "\n"
        if not example_events:
            example_events = self.example_events()

        exampled_message = self._generate_prompt_with_examples(example_events)

        ai_messages.append({
            "role": "user", "content": ai_message + exampled_message
        })
