    ...
    def _generate_prompt_with_events(self, events_list, max_events):
        ...
        instructions = (
            """Твоя задача — оценить каждое мероприятие по нескольким критериям на 10-балльной шкале. Вот критерии оценки:
                        1. Уникальность: Насколько мероприятие предлагает что-то новое и необычное? События, которые часто повторяются, получают меньший балл, пересмотри во всей предложенной подборке много ли похожего мероприятия будут ещё.
